...
/**
 * Sync adapter service bound to the calendar authority. Performs CalDAV sync.
 */
class CalendarSyncService : Service() {
    override fun onCreate() {
        ...
        synchronized(lock) {
            if (syncAdapter == null) {
                syncAdapter = CalendarSyncAdapter(applicationContext)
            }
        }
    }
...
class CalendarSyncAdapter(context: Context) : AbstractThreadedSyncAdapter(context, true) {
    override fun onPerformSync(
        ...
        runCatching { SouveraSyncManager(context).syncCalendars(account) }
